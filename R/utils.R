...
#' @param alpha significance threshold for displaying spikes 
#' @param xlims optional parameter to specify the x-axis limits
#' @param  ... to be passed to methods
#' @export
#' @examples 
...
#' ### with a selective p-value < alpha (default to be 0.05).
...
#' @import graphics
plot.spike_inference <- function(x, alpha = 0.05, xlims = NULL, ...) {
  
  ind <- 1:length(x$dat)
  rng <- range(c(x$dat))
  ylims <- rng
  hh <- 0.03 * diff(ylims)
  ylims[1] <- ylims[1]-hh*2
  if (is.null(xlims)){
    plot(ind, x$dat, cex = 0.5, pch = 20, col = "darkgrey", ylab = "", ylim = ylims, xlab = "Index")
  } else {
    plot(ind, x$dat, cex = 0.5, pch = 20, col = "darkgrey", ylab = "", ylim = ylims, xlim = xlims, xlab = "Time")
...
#' @param target_firing_rate Numeric; a number between 0 to 1 indicating the average probability of firing in each timestep
...
#' @export 
estimate_spike_by_firing_rate <- function(dat, decay_rate, target_firing_rate, 
